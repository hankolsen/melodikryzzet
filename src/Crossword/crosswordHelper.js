...
const emptyAll = (cells) => {
  cells.map((row) => row.forEach((column) => {
    if (column) {
      column.text = '';
    }
  }));
...
const createCrossword = (crosswordId) => new Promise((resolve, reject) => fetch(`/.netlify/functions/crosswords/${crosswordId}`)
...
const cellIsStartingWord = ({ cell, direction }) => cell
  && cell[direction]
  && cell[direction].find((id) => id.startsWith(cell.number));


const cellContainsOtherDirection = ({ currentCell, direction }) => (direction === 'across' && currentCell.down)
